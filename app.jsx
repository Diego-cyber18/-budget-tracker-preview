// Globals provided by index.html (React + Recharts via CDN)
const { useState, useEffect } = React;
const { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Legend, Cell } = Recharts;
...
function App() {
  ...
  useEffect(() => {
    async function load() {
      try { const h = localStorage.getItem("bgt-history"); if (h) setHistory(JSON.parse(h)); } catch {}
      try { const p = localStorage.getItem("bgt-plan"); if (p) setPlan(JSON.parse(p)); } catch {}
      try {
      ...
      try { const n = localStorage.getItem("bgt-names"); if (n) setNames(JSON.parse(n)); } catch {}
      try { const l = localStorage.getItem("bgt-lang"); if (l) setLang(l); } catch {}
      try { const u = localStorage.getItem("bgt-uscite"); if (u) setUscite(JSON.parse(u)); } catch {}
      try { const b = localStorage.getItem("bgt-bollette"); if (b) setBollette(JSON.parse(b)); } catch {}
      try { const p = localStorage.getItem("bgt-pellet"); if (p) setPelletLog(JSON.parse(p)); } catch {}
      setLoaded(true);
    }
    load();
  }, []);
  ...
  const importData = async (jsonStr) => {
    try {
      ...
      if (data.names)     { setNames(data.names);         localStorage.setItem("bgt-names", JSON.stringify(data.names)); }
      if (data.history)   { setHistory(data.history);     localStorage.setItem("bgt-history", JSON.stringify(data.history)); }
      if (data.plan)      { setPlan(data.plan);           localStorage.setItem("bgt-plan", JSON.stringify(data.plan)); }
      if (data.uscite)    { setUscite(data.uscite);       localStorage.setItem("bgt-uscite", JSON.stringify(data.uscite)); }
      if (data.bollette)  { setBollette(data.bollette);   localStorage.setItem("bgt-bollette", JSON.stringify(data.bollette)); }
      if (data.pelletLog) { setPelletLog(data.pelletLog); localStorage.setItem("bgt-pellet", JSON.stringify(data.pelletLog)); }
      setImportStatus("ok");
